...
// Source metadata for rendering (icons and display info)
const SOURCE_META: Record<string, {
    ...
    hackernews: {
        ...
        color: "text-orange-400",
        icon: <span className="w-4 h-4 bg-orange-500 text-white text-[10px] font-bold flex items-center justify-center rounded">Y</span>,
    },
    github: {
        name: "GitHub",
...
// Default meta for unknown sources
const DEFAULT_META = {
    name: "Source",
    title: "Top Items",
    color: "text-gray-200",
...
function SourceSection({ sourceId, items }: { sourceId: string; items: BreakdownItem[] }) {
    ...
    const meta = SOURCE_META[sourceId] || { ...DEFAULT_META, name: sourceId, title: `Top ${sourceId}` };

    return (
        <div>
            <div className="flex items-center gap-2 mb-4">
                {meta.icon}
                <h4 className={`text-sm font-medium ${meta.color}`}>{meta.title}</h4>
            </div>
